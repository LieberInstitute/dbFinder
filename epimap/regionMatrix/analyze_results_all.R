...
save(ensemblAnno, file = file.path(resdir, 'ensemblAnno_all.Rdata'))
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_ensemblVenn.pdf')), width = 10,
    height = 10)
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_ensemblVenn_percent.pdf')),
    width = 10, height = 10)
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_ensemblVenn_bp.pdf')), width = 10,
    height = 10)
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_ensemblVenn_bp_percent.pdf')),
    width = 10, height = 10)
vennDiagram_custom(venn_bp, 
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_dbPeaks_PCA_byAnno.pdf')))
...
pdf(file.path(plotdir, paste0(opt$histone, '_all_dbPeaks_PCsbyGroup_byAnno.pdf')),
    width = 11)
palette(brewer.pal(4, 'Paired'))
par(mar=c(14,6,2,2))
set.seed(20160514)
...
save(ssMat, lab, file = file.path(resdir, paste0('ssMat_all_', opt$histone,
    '.Rdata')), compress=TRUE)

## Boxplot by variables
pdf(file.path(plotdir, paste0(opt$histone, '_all_boxplots_overall.pdf')),
    width = 12, height = 5)
par(mar=c(9,5,2,2))
...
save(ssMat2, lab2, file = file.path(resdir, paste0('ssMat2_all_', opt$histone,
    '_noCellType.Rdata')), compress=TRUE)

## Boxplot by variables
pdf(file.path(plotdir, paste0(opt$histone, '_all_boxplots_overall_noCellType.pdf')),
    width = 12, height = 5)
par(mar=c(9,5,2,2))
palette(brewer.pal(7, "Dark2"))
